URM Simulator 是一个Python库，用于模拟无限寄存器机（Unlimited Register Machine，URM）的操作。URM是理论计算机科学中的一个简化模型，它通过一系列基本操作来模拟计算过程。这个库提供了一种方式来定义URM程序、执行这些程序，并观察寄存器中值的变化。
## 主要功能

- **支持URM的四种基本操作**：零（Zero）、后继（Successor）、复制（Copy）和跳转（Jump）。
- **程序执行与跟踪**：提供了用于执行URM程序并跟踪其执行过程的仿真器。
- **自定义URM程序**：允许用户定义和执行自定义URM程序，以及检查程序执行的结果。
## 高级功能

- **size函数**：计算URM程序中指令的数量。这对于分析程序复杂度和调试有帮助。
- **haddr函数**：找出URM程序中使用的最高寄存器地址。这对于确定程序需要多少寄存器非常重要。
- **normalize方法**：标准化URM程序，确保所有跳转操作都指向有效的指令行。这有助于优化程序结构和预防错误。
- **concat方法**：将两个URM程序连接成一个单一程序。这对于构建复杂逻辑或重用现有代码非常有用。
- **reloc方法**：根据指定的映射重定位URM程序中的寄存器地址。这在需要调整程序以适应特定寄存器配置时非常有用。
## 安装
